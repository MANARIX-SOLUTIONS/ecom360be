...
sales, and low-stock rows are scoped to that store. The following values remain tenant-wide:
...
It returns completed-sale revenue and count, average basket, expenses, profit, store revenue
shares, low-stock rows, and the top 10 products. The requested start is subject to plan retention,
