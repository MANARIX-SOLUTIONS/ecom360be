...
public class StoreService {
        ...
        subscriptionService.getPlanForBusiness(p.businessId()).ifPresent(plan -> {
            if (!plan.isUnlimited(plan.getMaxStores())) {
                ...
                if (count >= plan.getMaxStores()) {
                    throw new BusinessRuleException("Limite du plan atteinte : maximum " + plan.getMaxStores() + " magasin(s). Passez à un plan supérieur.");
                }
            }
        });
    ...
    public List<StoreResponse> list(UserPrincipal p) {
        requireBiz(p);
        return storeRepository.findByBusinessIdAndIsActive(p.businessId(), true).stream().map(this::map).toList();
    }
    public StoreResponse update(UUID id, StoreRequest req, UserPrincipal p) {
        requireBiz(p); Store s = find(id, p);
        s.setName(req.name()); s.setAddress(req.address()); s.setPhone(req.phone()); s.setIsActive(req.isActive());
        return map(storeRepository.save(s));
    }
    public void delete(UUID id, UserPrincipal p) { requireBiz(p); storeRepository.delete(find(id, p)); }

    private Store find(UUID id, UserPrincipal p) {
        return storeRepository.findById(id).filter(s -> s.belongsTo(p.businessId()))
    ...
    private StoreResponse map(Store s) {
        return new StoreResponse(s.getId(), s.getBusinessId(), s.getName(), s.getAddress(), s.getPhone(), s.getIsActive(), s.getCreatedAt(), s.getUpdatedAt());
    }
}
