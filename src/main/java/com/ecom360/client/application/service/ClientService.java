...
public class ClientService {
    ...
    public ClientService(ClientRepository clientRepo, ClientPaymentRepository paymentRepo, SubscriptionService subscriptionService) {
        ...
        subscriptionService.getPlanForBusiness(p.businessId()).ifPresent(plan -> {
            if (!plan.isUnlimited(plan.getMaxClients())) {
                ...
                if (count >= plan.getMaxClients()) {
                    throw new BusinessRuleException("Limite du plan atteinte : maximum " + plan.getMaxClients() + " client(s). Passez à un plan supérieur.");
                }
            }
        });
        Client c = new Client();
    ...
    @Transactional
    public ClientPaymentResponse recordPayment(UUID clientId, ClientPaymentRequest r, UserPrincipal p) {
        requireBiz(p);
        subscriptionService.getPlanForBusiness(p.businessId()).ifPresent(plan -> {
            if (!Boolean.TRUE.equals(plan.getFeatureClientCredits())) {
                throw new BusinessRuleException("Crédits clients non inclus dans votre plan. Passez à un plan supérieur.");
            }
        });
        ...
        return new ClientPaymentResponse(
            pay.getId(), pay.getClientId(), pay.getStoreId(), pay.getUserId(),
            pay.getAmount(), pay.getPaymentMethod(), pay.getNote(), pay.getCreatedAt()
        );
    }
    ...
    public Page<ClientPaymentResponse> getPayments(UUID clientId, UserPrincipal p, Pageable pg) {
        requireBiz(p);
        find(clientId, p);
        return paymentRepo.findByClientIdOrderByCreatedAtDesc(clientId, pg)
            .map(pay -> new ClientPaymentResponse(
                pay.getId(), pay.getClientId(), pay.getStoreId(), pay.getUserId(),
                pay.getAmount(), pay.getPaymentMethod(), pay.getNote(), pay.getCreatedAt()
            ));
    }
    ...
    private Client find(UUID id, UserPrincipal p) {
        return clientRepo.findByBusinessIdAndId(p.businessId(), id)
    ...
    private ClientResponse map(Client c) {
        return new ClientResponse(
            c.getId(), c.getBusinessId(), c.getName(), c.getPhone(), c.getEmail(),
            c.getAddress(), c.getNotes(), c.getCreditBalance(), c.getIsActive(),
