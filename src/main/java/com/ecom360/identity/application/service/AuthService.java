...
public class AuthService {
    ...
    public AuthService(UserRepository userRepository, BusinessRepository businessRepository, BusinessUserRepository businessUserRepository, PasswordResetRepository passwordResetRepository, SubscriptionService subscriptionService, AuditLogService auditLogService, PasswordEncoder passwordEncoder, JwtService jwtService, JwtProperties jwtProperties, EmailService emailService) {
    ...
    public AuthResponse login(LoginRequest request) {
        User user = userRepository.findByEmail(request.email()).orElseThrow(() -> new BusinessRuleException("Invalid email or password"));
...
//        }
        ...
        BusinessUser active = memberships.stream().filter(bu -> bu.isAccepted() && bu.isActive()).findFirst().orElseThrow(() -> new BusinessRuleException("No active business membership found"));

        auditLogService.logAsync(active.getBusinessId(), user.getId(), "LOGIN", "Auth", user.getId(), java.util.Map.of("email", user.getEmail()));
    ...
    public AuthResponse register(RegisterRequest request) {
        ...
        User user = User.create(request.fullName(), request.email(), passwordEncoder.encode(request.password()), request.phone());
        ...
        auditLogService.logAsync(business.getId(), user.getId(), "REGISTER", "Auth", user.getId(), java.util.Map.of("email", user.getEmail(), "businessName", business.getName()));
    ...
    public AuthResponse refreshToken(RefreshTokenRequest request) {
        ...
        User user = userRepository.findById(claims.userId()).orElseThrow(() -> new ResourceNotFoundException("User", claims.userId()));
        if (!user.isActive()) {
            throw new BusinessRuleException("Account is deactivated");
        }

        BusinessUser bu = businessUserRepository.findByUserId(user.getId()).stream().filter(b -> b.isAccepted() && b.isActive()).findFirst().orElse(null);
    ...
    public void changePassword(UUID userId, UUID businessId, ChangePasswordRequest request) {
        ...
        if (businessId != null) {
            auditLogService.logAsync(businessId, userId, "PASSWORD_CHANGE", "Auth", userId, java.util.Map.of());
        }
    }
    ...
    public void resetPassword(ResetPasswordRequest request) {
        ...
        PasswordReset pr = passwordResetRepository.findByTokenHashAndUsedFalse(tokenHash).orElseThrow(() -> new BusinessRuleException("Invalid or expired reset token"));
        if (!pr.isValid()) {
            throw new BusinessRuleException("Reset token has expired");
        }
        User user = userRepository.findById(pr.getUserId()).orElseThrow(() -> new ResourceNotFoundException("User", pr.getUserId()));
        user.setPasswordHash(passwordEncoder.encode(request.newPassword()));
        userRepository.save(user);
        pr.markUsed();
        passwordResetRepository.save(pr);
    ...
    private AuthResponse buildAuthResponse(User user, UUID businessId, String role) {
        ...
        String accessToken = jwtService.generateAccessToken(user.getId(), user.getEmail(), businessId, effectiveRole, user.isPlatformAdmin());
        String refreshToken = jwtService.generateRefreshToken(user.getId());
        String planSlug = businessId != null ? subscriptionService.getPlanSlugForBusiness(businessId) : null;
        return new AuthResponse(accessToken, refreshToken, jwtProperties.getExpirationMs() / 1000, user.getId(), user.getEmail(), user.getFullName(), businessId, effectiveRole, planSlug);
    }
}
