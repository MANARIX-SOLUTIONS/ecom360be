...
//        }
        ...
        BusinessUser active = memberships.stream()
    ...
    public AuthResponse register(RegisterRequest request) {
        ...
        User user = User.create(request.fullName(), request.email(),
    ...
    public AuthResponse refreshToken(RefreshTokenRequest request) {
        ...
        BusinessUser bu = businessUserRepository.findByUserId(user.getId()).stream()
    ...
    /**
     * Creates a password reset token for the given user (e.g. for invitation flow).
    ...
    public void resetPassword(ResetPasswordRequest request) {
        ...
        PasswordReset pr = passwordResetRepository.findByTokenHashAndUsedFalse(tokenHash)
    ...
    private AuthResponse buildAuthResponse(User user, UUID businessId, String role) {
        String accessToken = jwtService.generateAccessToken(user.getId(), user.getEmail(), businessId, role, user.isPlatformAdmin());
        String refreshToken = jwtService.generateRefreshToken(user.getId());
        String planSlug = subscriptionService.getPlanSlugForBusiness(businessId);
        return new AuthResponse(accessToken, refreshToken, jwtProperties.getExpirationMs() / 1000,
                user.getId(), user.getEmail(), user.getFullName(), businessId, role, planSlug);
    }
}
