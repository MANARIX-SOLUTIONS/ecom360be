...
public class ApiKeyController {
    ...
    public ResponseEntity<ApiKeyResponse> get(@PathVariable UUID id, @AuthenticationPrincipal UserPrincipal p) {
        return ResponseEntity.ok(apiKeyService.getById(id, p));
    }

    @DeleteMapping("/{id}")
    @Operation(summary = "Revoke API key")
    public ResponseEntity<Void> revoke(@PathVariable UUID id, @AuthenticationPrincipal UserPrincipal p) {
