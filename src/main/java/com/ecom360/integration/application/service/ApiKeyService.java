...
public class ApiKeyService {
    ...
    public ApiKeyResponse create(ApiKeyRequest request, UserPrincipal p) {
        ...
        return new ApiKeyResponse(apiKey.getId(), apiKey.getBusinessId(), apiKey.getLabel(),
                apiKey.getPermissions(), apiKey.getExpiresAt(), apiKey.getIsActive(), apiKey.getCreatedAt(), rawKey);
    }
    ...
    private ApiKey find(UUID id, UserPrincipal p) {
        return apiKeyRepository.findByBusinessIdAndId(p.businessId(), id)
                .orElseThrow(() -> new ResourceNotFoundException("ApiKey", id));
    }

    private ApiKeyResponse toResponse(ApiKey a) {
        return ApiKeyResponse.withoutRawKey(a.getId(), a.getBusinessId(), a.getLabel(), a.getPermissions(),
                a.getExpiresAt(), a.getIsActive(), a.getCreatedAt());
    }
