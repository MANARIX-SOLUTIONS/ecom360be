...
public class AdminUserController {
    ...
    public ResponseEntity<PageResponse<AdminUserResponse>> list(
        ...
        return ResponseEntity.ok(PageResponse.of(
                adminUserService.list(p, page, Math.min(size, ApiConstants.MAX_PAGE_SIZE), search, status, role)));
    }

    @PostMapping("/invite")
    @Operation(summary = "Invite user to business (platform admin)")
    public ResponseEntity<AdminUserResponse> invite(@Valid @RequestBody AdminInviteRequest req,
                                                    @AuthenticationPrincipal UserPrincipal p) {
