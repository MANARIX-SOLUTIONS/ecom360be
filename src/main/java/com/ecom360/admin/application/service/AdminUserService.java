...
public class AdminUserService {
    ...
    public Page<AdminUserResponse> list(UserPrincipal p, int page, int size, String search, String status, String role) {
        Pageable pageable = PageRequest.of(page, Math.min(size, 100), Sort.by(Sort.Direction.DESC, "createdAt"));
        String q = (search != null && !search.isBlank()) ? search.trim() : null;
        Boolean active = null;
        if (status != null && "active".equals(status.trim())) active = true;
        else if (status != null && ("disabled".equals(status.trim()) || "inactive".equals(status.trim()))) active = false;
    ...
    private Map<UUID, String> loadPrimaryRole(List<UUID> userIds) {
        return userIds.stream()
                ...
                .collect(Collectors.toMap(BusinessUser::getUserId, bu -> formatRole(bu.getRole()), (a, b) -> a));
    }

    private Map<UUID, String> loadPrimaryBusiness(List<UUID> userIds) {
        return userIds.stream()
                .flatMap(uid -> businessUserRepository.findByUserId(uid).stream().findFirst().stream())
                .collect(Collectors.toMap(BusinessUser::getUserId, bu -> {
    ...
    public AdminUserResponse invite(AdminInviteRequest req, UserPrincipal p) {
        ...
        Optional<Subscription> subOpt = subscriptionRepository.findFirstByBusinessIdAndStatusInOrderByCreatedAtDesc(
                req.businessId(), List.of("active", "trialing"));
        subOpt.flatMap(sub -> planRepository.findById(sub.getPlanId())).ifPresent(plan -> {
            if (!plan.isUnlimited(plan.getMaxUsers())) {
                int count = businessUserRepository.findByBusinessIdAndIsActive(req.businessId(), true).size();
                if (count >= plan.getMaxUsers()) {
                    throw new BusinessRuleException("Limite du plan atteinte : maximum " + plan.getMaxUsers() + " utilisateur(s) pour cette entreprise.");
                }
            }
        });
        ...
        if (businessUserRepository.existsByBusinessIdAndUserId(req.businessId(), user.getId())) {
            throw new ResourceAlreadyExistsException("User", req.email() + " est déjà membre de cette entreprise");
        }

        String roleSlug = mapRoleToSlug(req.role());
        BusinessUser bu = BusinessUser.create(req.businessId(), user.getId(), roleSlug != null ? roleSlug : "caissier");
    ...
    public void setStatus(UUID userId, boolean active, UserPrincipal p) {
        User u = userRepository.findById(userId)
                .orElseThrow(() -> new com.ecom360.shared.domain.exception.ResourceNotFoundException("User", userId));
        if (active) {
            u.activate();
        } else {
            u.deactivate();
