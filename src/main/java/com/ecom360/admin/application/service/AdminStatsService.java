...
public class AdminStatsService {
  ...
  public AdminStatsResponse getStats(UserPrincipal p) {
    ...
    List<Object[]> revenueByBusiness =
        saleRepository.sumTotalByBusinessIdBetween(monthStart, monthEnd);
    long monthlyRevenue =
        revenueByBusiness.stream().mapToLong(row -> ((Number) row[1]).longValue()).sum();

    Map<UUID, Long> revenueMap =
        revenueByBusiness.stream()
            .collect(Collectors.toMap(row -> (UUID) row[0], row -> ((Number) row[1]).longValue()));

    List<AdminStatsResponse.PlanCount> planDistribution =
        computePlanDistribution((int) businessesCount);
    List<AdminStatsResponse.TopBusiness> topBusinesses = computeTopBusinesses(revenueMap);

    return new AdminStatsResponse(
        businessesCount, usersCount, storesCount, monthlyRevenue, planDistribution, topBusinesses);
  }
  ...
  private List<AdminStatsResponse.PlanCount> computePlanDistribution(int totalBusinesses) {
    ...
    List<Subscription> active =
        subscriptionRepository.findAll().stream().filter(Subscription::isActive).toList();
  ...
  private List<AdminStatsResponse.TopBusiness> computeTopBusinesses(Map<UUID, Long> revenueMap) {
    ...
    if (topBizIds.isEmpty()) {
      List<Business> recent =
          businessRepository
              .findAllByOrderByCreatedAtDesc(org.springframework.data.domain.PageRequest.of(0, 5))
              .getContent();
    ...
    for (UUID bizId : topBizIds) {
      ...
      result.add(
          new AdminStatsResponse.TopBusiness(
              b.getName(),
