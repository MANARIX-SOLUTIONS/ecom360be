...
public interface SaleRepository extends JpaRepository<Sale, UUID> {
    ...
    /**
     * Revenue (SUM total) and count of completed sales for a business over a date
     * range, optionally scoped to a single store. Returns a list with at most one
     * row {@code [revenue, count]} (List avoids Spring Data wrapping a bare
     * {@code Object[]} multi-column result).
     */
    @Query("SELECT COALESCE(SUM(s.total), 0), COUNT(s) FROM Sale s"
    ...
    List<Object[]> sumRevenueAndCountBetween(
