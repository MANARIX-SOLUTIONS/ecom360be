...
public class ExpenseService {
  ...
  public Page<ExpenseResponse> list(
      UserPrincipal p, UUID categoryId, UUID storeId, Integer month, Integer year, Pageable pg) {
    requireBiz(p);
    permissionService.require(p, Permission.EXPENSES_READ);
  ...
  private LocalDate expenseRetentionMin(UserPrincipal p) {
    return subscriptionService
        ...
        .map(pl -> LocalDate.now(ZoneId.systemDefault()).minusMonths(pl.getDataRetentionMonths()))
        .orElse(null);
  }
