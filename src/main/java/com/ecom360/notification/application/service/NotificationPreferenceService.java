...
public class NotificationPreferenceService {
  ...
  public List<NotificationPreferenceResponse> getPreferences(UserPrincipal p) {
    ...
    Map<String, Boolean> byType =
        existing.stream().collect(Collectors.toMap(NotificationPreference::getType, NotificationPreference::getEnabled));
        ...
        .map(
            type ->
                new NotificationPreferenceResponse(
                    type, byType.getOrDefault(type, true)))
        .toList();
  }
  ...
  public boolean isEnabled(UUID userId, String type) {
    return repo.findByUserIdAndType(userId, type).map(NotificationPreference::getEnabled).orElse(true);
  }
}
